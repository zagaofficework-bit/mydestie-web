...
{     
  ...
  while($data_1 = mysqli_fetch_assoc($sql_1))
  {
    ...
    $row_1['place_name'] = htmlspecialchars_decode($data_1['place_name']);
    ...
    $row_1['place_description'] = stripslashes(htmlspecialchars_decode($data_1['place_description']));
    $row_1['place_address'] = htmlspecialchars_decode($data_1['place_address']);
  ...
  while($data_2 = mysqli_fetch_assoc($sql_2))
  {
    ...
    $row_2['place_name'] = htmlspecialchars_decode($data_2['place_name']);
    ...
    $row_2['place_description'] = stripslashes(htmlspecialchars_decode($data_2['place_description']));
    $row_2['place_address'] = htmlspecialchars_decode($data_2['place_address']);
...
else if($get_method['method_name']=='featured_place')   
{
  ...
  while($data = mysqli_fetch_assoc($sql))
  {
    ...
    $row['place_name'] = htmlspecialchars_decode($data['place_name']);
    ...
    $row['place_description'] = stripslashes(htmlspecialchars_decode($data['place_description']));
    $row['place_address'] = htmlspecialchars_decode($data['place_address']);
...
else if($get_method['method_name']=="get_nearby_place")   
{
  ...
  else if($get_method['method_name']=="get_favourite_list")   
  {
    ...
    { 
      ...
      $row['place_thumb_image'] = get_thumb('images/'.$data['place_image'],'500*400');
      $row['place_video'] = $data['place_video'];
      $row['place_description'] = htmlspecialchars_decode($data['place_description']);
      $row['place_address'] = htmlspecialchars_decode($data['place_address']);
      $row['place_email'] = $data['place_email'];
      $row['place_phone'] = $data['place_phone'];
      $row['place_website'] = $data['place_website'];
  ...
  else if($get_method['method_name']=="place_ratings")
  {
    ...
    $ip = filter_var($get_method['ip'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    ...
    $message = filter_var($get_method['message'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    ...
    if($flag==0){
      ...
      $result = mysqli_query($mysqli,"SELECT * FROM tbl_rating WHERE `post_id`='$post_id' AND `user_id`='$user_id'"); 
        ...
        if(count((array)$rate_db)){
          $rate_times = count((array)$rate_db);
          $sum_rates = array_sum($sum_rates);
          $rate_value = $sum_rates/$rate_times;
          $rate_bg = (($rate_value)/5)*100;
        }else{
  ...
  // Get category list end
  ...
  else if ($get_method['method_name'] == "delete_user_account") 
	{
		...
		if ($result->num_rows > 0) {
			...
			Delete('tbl_active_log', 'user_id=' . $id);
			Delete('tbl_rating', 'user_id=' . $id);
  ...
  // Get app details start  
    ...
    while($data = mysqli_fetch_assoc($sql))
    {
      ...
      $row['interstitial_ad_type'] = $data['android_ad_network'];

      if ($data['android_ad_network'] == 'facebook') {
        $row['interstitial_ad_id'] = $data['interstital_facebook_id'];
      } else if ($data['android_ad_network'] == 'admob') {
        $row['interstitial_ad_id'] = $data['interstital_ad_id'];
      } else if ($data['android_ad_network'] == 'unityds') {
        $row['interstitial_ad_id'] = $data['interstitial_unity_id'];
      } else if ($data['android_ad_network'] == 'applovins') {
      ...
      $row['banner_ad_type'] = $data['android_ad_network'];
      ...
      if($data['android_ad_network']=='unityds'){
        ...
        $row['nativ_ad'] = $data['nativ_ad'];
      }
       
      $row['nativ_ad_type'] = $data['android_ad_network'];

      if ($data['android_ad_network'] == 'facebook') {
        $row['nativ_ad_id'] = $data['nativ_facebook_id'];
      } else if ($data['android_ad_network'] == 'admob') {
        $row['nativ_ad_id'] = $data['nativ_ad_id'];
      } else if ($data['android_ad_network'] == 'unityds') {
        $row['nativ_ad_id'] = $data['interstitial_unity_id'];
      } else if ($data['android_ad_network'] == 'applovins') {
        $row['nativ_ad_id'] = $data['nativ_applovin_id'];
      } else if ($data['android_ad_network'] == 'wortise') {
        $row['nativ_ad_id'] = $data['native_wortise_id'];
      } else if ($data['android_ad_network'] == 'startapp') {
        $row['nativ_ad_id'] = '';
      }

      $row['nativ_ad_position'] = $data['nativ_ad_click'];
